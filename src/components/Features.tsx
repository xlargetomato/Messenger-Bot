...
export default function Features() {
  ...
  const featureVisuals = [
    (
      <div className="mt-4 bg-slate-900 p-3 border border-slate-700">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="w-16 h-8 bg-slate-800 flex items-center justify-center text-[10px] text-gray-300">Message</div>
          <div className="w-4 h-px bg-slate-600" />
          <div className="w-16 h-8 bg-slate-800 flex items-center justify-center text-[10px] text-gray-300">Reply</div>
        </div>
    ...
    (
      <div className="mt-4 bg-slate-900 p-3 border border-slate-700">
        ...
        <p className="text-[10px] text-gray-500">{language === 'ar' ? 'جاري التحليل...' : 'Analyzing intent...'}</p>
      </div>
    ),
    (
      <div className="mt-4 bg-slate-900 p-3 border border-slate-700">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="w-8 h-8 bg-slate-800 flex items-center justify-center"><Users className="w-4 h-4 text-gray-300" /></div>
          </div>
        </div>
        <div className="flex items-center gap-1 mt-2">
          <Check className="w-3 h-3 text-emerald-500" />
          <p className="text-[10px] text-emerald-500">{language === 'ar' ? 'تم التحويل' : 'Transferred'}</p>
        </div>
      </div>
    ),
    (
      <div className="mt-4 bg-slate-900 p-3 border border-slate-700">
        <div className="flex items-end gap-1 h-12">
          <div className="w-3 bg-slate-700" style={{ height: '40%' }} />
          <div className="w-3 bg-slate-700" style={{ height: '60%' }} />
        ...
        <p className="text-[10px] text-gray-500 mt-2">{language === 'ar' ? 'المحادثات هذا الأسبوع' : 'Conversations this week'}</p>
      </div>
    ),
    (
      <div className="mt-4 bg-slate-900 p-3 border border-slate-700">
        <div className="flex flex-wrap gap-2">
          <span className="px-2 py-1 bg-slate-800 text-xs text-gray-400">EN</span>
          <span className="px-2 py-1 bg-azure-600 text-xs text-white">AR</span>
          <span className="px-2 py-1 bg-slate-800 text-xs text-gray-400">FR</span>
          <span className="px-2 py-1 bg-slate-800 text-xs text-gray-400">ES</span>
        </div>
      </div>
    ),
    (
      <div className="mt-4 bg-slate-900 p-3 border border-slate-700">
        <div className="flex flex-wrap gap-2">
          <button className="px-3 py-1.5 bg-azure-500/20 border border-azure-500/30 text-[10px] text-azure-300 flex items-center gap-1">
            <Check className="w-3 h-3" />{language === 'ar' ? 'نعم' : 'Yes'}
          </button>
          <button className="px-3 py-1.5 bg-slate-800 border border-slate-700 text-[10px] text-gray-400">{language === 'ar' ? 'لا' : 'No'}</button>
          <button className="px-3 py-1.5 bg-slate-800 border border-slate-700 text-[10px] text-gray-400">{language === 'ar' ? 'المزيد...' : 'More...'}</button>
        </div>
      </div>
    ),
  ]
